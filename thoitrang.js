const man = [
  ...
  function loadman(){
      sunshineMan=JSON.parse(localStorage.getItem('listman')) || [];
  }
  function loadwomen(){
    sunshineWomen=JSON.parse(localStorage.getItem('listwomen'));
}
  function Man() {
    var html = "";
    for (i in man) {
  ...
  function createProduct() {
    ...
    if (Select === "man") {
      man.push(newProduct);
      localStorage.setItem("listman", JSON.stringify(man));
      listman = JSON.parse(localStorage.getItem(man))
...
// }
  ...
  function showAllProducts() {
    loadman(); // Load danh sách sản phẩm thức ăn từ localStorage
    var html = "";
    html += Man(); // Hiển thị danh sách sản phẩm thức ăn
    loadwomen(); // Load danh sách sản phẩm đồ uống từ localStorage
    html += Women(); // Hiển thị danh sách sản phẩm đồ uống
    document.getElementById("tbl").innerHTML = html;
